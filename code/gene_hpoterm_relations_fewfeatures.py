...
KW = ["involved", "increased", "express", "expression", "reduced", "deﬁned", "found", "contributing", "established", "decreased", "associated"]
KWS = []
stemmer = SnowballStemmer("english")

for word in KW: KWS.append(stemmer.stem(word))
...
def add_features(relation, gene_mention, hpoterm_mention, sentence):
    ...
    if betw_end - betw_start - 1 < 15:
        ...
        for word in sentence.words[max(0, betw_start - 3) : betw_end + 3]:
          if stemmer.stem(word.lemma) in KWS:
            relation.add_feature(inv + "RELATION_INDICATOR")
            break
